...
try:
    ...
    for sq in sequences_to_analyze:
                ...
                if c_over_g > 1:
                    if re.search(pattern1, w):
                        ...
                        if ctrl > 0 or re.search(pattern_pause_site1, s):
                            genome = genome[:x1] + f"RUT{cod}" + genome[x1:x2] + genome[
                                                                                 x2:x2 + 150] + f"RUT{cod}" + genome[
                                                                                                              x2 + 150:]
                            num += 1
                            predictions += 1
                            scale = x2 + 150 - j - 1
                ...
                if c_over_g > 1:
                    if re.search(pattern2, w):
                        ...
                        if ctrl > 0 or re.search(pattern_pause_site2, s):
                            genome = genome[:x1 - 150] + f"RUT{cod}" + genome[x1 - 150:x1] + genome[
                                                                                             x1:x2] + f"RUT{cod}" + genome[
                                                                                                                    x2:]
                            num += 1
                            predictions += 1
                            scale = x2 + 150 - j - 1
        ...
        # write the modified genome to a new file
        with open(f"{sq}_modified.fasta", "w") as f:
            f.write(f">{sq}\n")
            f.write(f"{genome}\n")
